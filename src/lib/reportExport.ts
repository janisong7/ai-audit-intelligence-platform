...
export function buildAuditReportCsv({ tickets, assessments, decisions, trail, reviewer }: ReportExportInput) {
  ...
  const columns = ['Ticket ID', 'Ticket Type', 'Priority', 'Title', 'Created Date', 'Resolution Date', 'SLA Status', 'Approval Status', 'Resolution Quality', 'Resolution Hours', 'Compliance Score', 'Risk Level', 'Evidence Coverage Index (%)', 'Findings', 'Recommendation', 'Anomaly Detected', 'Audit Status', 'Final Decision', 'Reviewer', 'Decision Date', 'Override Rationale'];
  const rows = tickets.map(ticket => {
    ...
    return [ticket.ticketId, ticket.ticketType, ticket.priority, ticket.title, ticket.createdDate, ticket.resolutionDate, ticket.slaStatus, ticket.approvalStatus, ticket.resolutionQuality, assessment?.resolutionHours ?? '', assessment?.complianceScore ?? '', assessment?.riskLevel ?? '', assessment?.assessmentConfidence ?? '', assessment?.findings.join(' | ') ?? '', assessment?.recommendations.join(' | ') ?? '', assessment?.anomalyDetected ? 'Yes' : 'No', decision?.status ?? 'Pending Review', decision?.kind ?? review?.decision ?? '', decision?.kind ? reviewer ?? '' : '', decision?.decisionDate ?? review?.occurredAt ?? '', decision?.rationale ?? review?.rationale ?? ''];
  });
